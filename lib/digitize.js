...
Digitize.prototype.fetch = function (sessionId, options, cb) {
	...
	var getOptions = {
		host: 'digitize.io',
		path: '/api/ocr?apiKey=' + o1.options.apiKey + '&sessionId=' + sessionId + '&format=' + format,
		method: 'GET'
	};
	...
	var req = https.request(getOptions, function(res) {
		...
		res.on('data', function (chunk) {
			...
			//try-catch will lead to issues if the OCRed content is a JSON object
			//and the user wants text or hOCR output

			//If an error is thrown, then chunk is not a JSON object.
			//This means that the output is a string.
			try {
				//For JSON, output is used to check the status of the processing (e.g. "In Progress").
				//json_output contains the JSON to be sent to the user
				output = JSON.parse(chunk);
				json_output = chunk.toString();
			}
			catch(err) {
		...
		res.on('end', function() {
			...
			if (output_type === 'Object') {
				if (output.message)
					...
					return cb (null, output.status, json_output);
				} else {
					o1._log('Status - ' + output.status + '\n');
